...
def compute_distance_travelled(message_list):
    ...
    for msg in message_list:
        if prev_position is None:
            ...
            distance += np.sqrt(
                    (current_position[0] - prev_position[0])**2 + (current_position[1] - prev_position[1])**2
            )
            distance_list.append(
                {
                    "timestamp": msg.timestamp,
...
def match_lookup(lookup_time, lookup_list, previous_index, max_search=100):
    ...
    while time_delta < 0 and i < previous_index + max_search:  # lookup_list time is before lookup_time
        ...
        if i == previous_index + max_search:
            print(f"[info: post_process] index={i}: max_search exceeded, returning latest item of lookup_list. Check message frequencies")
    ...
    if i - 2 > 0 and abs(lookup_list[i-2].timestamp.to_sec() - lookup_time) < abs(time_delta):
...
def error_horiz(groundtruth_msg, estimated_msg):
    return  np.sqrt(
        (groundtruth_msg.message.x - estimated_msg.message.x)**2 + (groundtruth_msg.message.y - estimated_msg.message.y)**2
    )

def error_yaw(groundtruth_msg, estimated_msg):
    return np.abs(
    groundtruth_msg.message.theta - estimated_msg.message.theta
)
